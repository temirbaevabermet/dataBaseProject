...
@Component
@FxmlView("/view/ClientsOrdersView.fxml")
public class ClientsOrdersController {
    ...
    private void loadFilteredList(String clientId) {
        try {
            ObservableList<OrdersListsModel> items = getItemsFromApi(clientId);
    ...
    private ObservableList<OrdersListsModel> getItemsFromApi(String clientId) throws Exception {
        List<OrdersListsModel> items = new ArrayList<>();

        try {
            URL url = new URL("http://localhost:8012/orders/client/" + clientId);
